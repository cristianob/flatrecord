...
import { deserializeStream } from '../../src/ts/geojson/featurecollection.js';
...
describe('geojson module', () => {
    describe('Geometry roundtrips', () => {
        it('Point', async () => {
            ...
            const actual = await takeAsync<IGeoJsonFeature>(deserializeStream(s));
        ...
        it('Point via stream', async () => {
            ...
            const actual = await takeAsync<IGeoJsonFeature>(deserializeStream(stream));
            expect(actual).to.deep.equal(expected.features);
        });
 ...
 (20 30, 35 35, 30 20, 20 30))))`);
        ...
        it('Heterogeneous geometries', async () => {
            ...
            const actual = await takeAsync<IGeoJsonFeature>(
                deserializeStream(serialize(expected, undefined, { writeSpatialIndex: false })),
            );
            expect(actual).to.deep.equal(expected.features);
        });
