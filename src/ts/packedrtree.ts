// Packed Hilbert R-tree streaming query. The level-bounds calculation
// and the node-range traversal are derived from "flatbush" by
...
/**
 * When streaming an R-tree, the search may want to merge two adjacent
 * fetches into a single range request if the gap between them is
 * smaller than this threshold (in bytes). Trade-off: lower = more
 * requests but less wasted bytes; higher = fewer requests but more
 * wasted bytes per gap.
 */
const EXTRA_REQUEST_THRESHOLD = 256 * 1024;
...
export async function* streamSearch(
    ...
    while (queue.length !== 0) {
        ...
        for (let nodeIdx = nodeRangeStartIdx; nodeIdx < nodeRangeEndIdx; nodeIdx++) {
            ...
            // extra request
            const extraRequestThresholdNodes = EXTRA_REQUEST_THRESHOLD / NODE_ITEM_BYTE_LEN;
